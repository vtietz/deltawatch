...
class DirectoryChangeTracker(FileSystemEventHandler):
    ...
    def __init__(self, console: Console, max_history: int = 1000, exclude_patterns: Optional[List[str]] = None):
        super().__init__()
        ...
        # Recent events queue
        self.recent_events: Deque[tuple] = deque(maxlen=max_history)  # (time, type, path, size_delta)
        ...
        if file_path:
            try:
                if event_type == "created":
                    ...
                    if os.path.exists(file_path):
                        new_size = os.path.getsize(file_path)
                        old_size = self.file_sizes.get(file_path, new_size)  # If unknown, assume no change
...
def create_display(tracker: DirectoryChangeTracker, args) -> Panel:
    ...
    if tracker.event_counts:
        event_summary = ", ".join([f"{k}: {v}" for k, v in tracker.event_counts.items()])
        header.add_row("Event Types:", event_summary)
    
    # Recent events table (only if --show-events flag is used)
    recent_table = None
    if args.show_events:
        recent_table = Table(title=f"🔥 Recent Events (last {args.event_count})", 
        ...
        for event_time, event_type, path, size_delta in tracker.get_recent_events(args.event_count):
    ...
    if args.minutes is None:
        ...
        dirs_title = f"📁 Top {args.top} Directories by Size Change (last {args.minutes} min)"
    
    dirs_table = Table(title=dirs_title, 
    ...
    return Panel(layout, title="[bold]Delta Watch[/bold]", 
                 border_style="blue")
